...
static void change_led_state(esp_ble_mesh_model_t *model,
...
static void handle_gen_onoff_msg(esp_ble_mesh_model_t *model,
    ...
    case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK:
        ...
        change_led_state(model, ctx, srv->state.onoff);
        break;
    default:
        break;
    }
...
static esp_err_t ble_mesh_init(void) {
    ...
    err = bt_mesh_set_device_name("M5C3-MESH-NW");
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "%s Fail to set name (err %d)", err);
        return err;
    }
...
void app_main(void) {
    ...
    // setup led strip
    ledstrip = malloc( sizeof( led_strip_t ));
